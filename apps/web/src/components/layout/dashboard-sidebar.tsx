...
const NAV_ITEMS = [
  ...
  { href: "/jobs", label: "Jobs", icon: Search, badge: "New" },
  { href: "/applications", label: "Applications", icon: Briefcase },
  { href: "/interviews", label: "Interviews", icon: Mic },
  { href: "/practice", label: "Practice", icon: Brain, badge: "AI" },
...
const MOBILE_NAV = [
  ...
  { href: "/resumes", label: "Resumes", icon: FileText },
  { href: "/interviews", label: "Interview", icon: Mic },
    ...
    <>
      ...
      <aside className="hidden md:flex w-60 flex-col border-r border-border/50 bg-card">
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5">
          {NAV_ITEMS.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4 bg-brand-500/20 text-brand-400 border-0">
                  {item.badge}
                </Badge>
              )}
              {isActive(item.href, item.exact) && <ChevronRight className="w-3 h-3 opacity-50" />}
            </Link>
          ))}
        </nav>
        ...
        <div className="mx-3 mb-3 px-3 py-2.5 rounded-lg bg-brand-500/10 border border-brand-500/20">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 opacity-70">Scanning 1,247 jobs for you</p>
        </div>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex md:hidden items-center justify-between px-4 py-3 border-b border-border/50 bg-card/95 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-brand-gradient flex items-center justify-center">
            <Bot className="w-4 h-4 text-white" />
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex md:hidden border-t border-border/50 bg-card/95 backdrop-blur-sm">
        {MOBILE_NAV.map((item) => {
