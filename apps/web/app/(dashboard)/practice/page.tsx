import { auth } from "@clerk/nextjs/server";
import { db } from "@/lib/db/client";
import { getOrCreateUser } from "@/lib/auth/get-or-create-user";
import { PracticeBoard } from "@/components/features/practice/practice-board";
import type { Metadata } from "next";

export const metadata: Metadata = { title: "Interview Practice" };

const DAILY_LIMITS = { FREE: 3, PRO: 25 } as const;

export default async function PracticePage() {
  const { userId: clerkId } = await auth();
  if (!clerkId) return null;

  await getOrCreateUser();

  const user = await db.user.findUnique({
    where: { clerkId },
    select: { id: true, plan: true },
  });
  if (!user) return null;

  const plan = (user.plan as "FREE" | "PRO") ?? "FREE";
  const dailyLimit = DAILY_LIMITS[plan] ?? 3;

  const todayStart = new Date();
  todayStart.setHours(0, 0, 0, 0);

  const usedToday = await db.aiGeneration.count({
    where: {
      userId: user.id,
      feature: "INTERVIEW_QUESTION",
      createdAt: { gte: todayStart },
      success: true,
    },
  });

  return <PracticeBoard plan={plan} dailyLimit={dailyLimit} usedToday={usedToday} />;
}
